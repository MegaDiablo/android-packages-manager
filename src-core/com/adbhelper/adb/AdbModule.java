...
public class AdbModule implements AdbConsts {
    ...
    private static final String CMD_SEND_TAP = "shell input tap %s %s";
    private static final String CMD_SEND_SWIPE = "shell input swipe %s %s %s %s";
    ...
    private static final String CMD_TAKE_SCREENSHOT = "exec-out screencap -p > %s";
    ...
    private static final String LOG_SEND_TAP_BY = "Sending tap by (%s, %s)";
    private static final String LOG_COMPLITE_SEND_TAP_BY = "Sended tap by (%s, %s)";
    private static final String LOG_SEND_SWIPE = "Sending swipe from (%s, %s) to (%s, %s)";
    private static final String LOG_COMPLITE_SEND_SWIPE = "Sended swipe";
    private static final String LOG_TAKE_SCREENSHOT = "Screen capture and save to %s";
    private static final String LOG_COMPLITE_TAKE_SCREENSHOT = "Screen capture saved to file";
     ...
     * @param device    - target device
     * @param pathApp   - path of file
     * @param autoStart - start package after install
     * @return Name of installed package
     */
    ...
    public void sendTap(final String device, final int x, final int y) {
        logAdb.info(String.format(LOG_SEND_TAP_BY, x, y));
        runAdb(device, String.format(CMD_SEND_TAP, x, y));
        logAdb.info(String.format(LOG_COMPLITE_SEND_TAP_BY, x, y));
    }

    public void sendTap(final AdbDevice device, final int x, final int y) {
        sendTap(device.getName(), x, y);
    }

    public void sendSwipe(final String device, final int fromX, final int fromY,final int toX, final int toY) {
        logAdb.info(String.format(LOG_SEND_SWIPE, fromX, fromY, toX, toY));
        runAdb(device, String.format(CMD_SEND_SWIPE, fromX, fromY, toX, toY));
        logAdb.info(LOG_COMPLITE_SEND_SWIPE);
    }

    public void sendSwipe(final AdbDevice device, final int fromX, final int fromY,final int toX, final int toY) {
        sendSwipe(device.getName(), fromX, fromY, toX, toY);
    }

    public void takeScreenshot(final String device, String path) {
        logAdb.info(String.format(LOG_TAKE_SCREENSHOT, path));
        String tmpDeviceFilePath = "/sdcard/apm-screenshot.png";
        runAdb(device, String.format("shell rm -f %s", tmpDeviceFilePath));
        runAdb(device, String.format("shell screencap -p %s", tmpDeviceFilePath));
        runAdb(device, String.format("pull %s %s", tmpDeviceFilePath, path));
        runAdb(device, String.format("shell rm -f %s", tmpDeviceFilePath));
        logAdb.info(LOG_COMPLITE_TAKE_SCREENSHOT);
    }

    public void takeScreenshot(final AdbDevice device, String path) {
        takeScreenshot(device.getName(), path);
    }
